...
class TrafficLight(context: Context, attrs: AttributeSet): View(context, attrs) {
    ...
    private fun drawYellowLight(canvas: Canvas) {
        paint.color = if (state == TrafficState.SLOW) {
            YELLOW_COLOR
        } else {
            GREY_COLOR
        }
        canvas.drawCircle(150f, 400f, 100f, paint)
    }

    private fun drawGreenLight(canvas: Canvas) {
        paint.color = if (state == TrafficState.GO) {
            GREEN_COLOR
        } else {
            GREY_COLOR
        }
        canvas.drawCircle(150f, 650f, 100f, paint)
    }
}
